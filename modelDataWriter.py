...
for index in range(DATASET_SIZE):
  ...
  if (alternatingIndex % 2 == 0):
    ...
    sheet1.write(row1, 1, tungstenHeatingRate)
    sheet1.write(row1, 2, biomassMass)

    datasetA.append([tarYield['yield'], tungstenHeatingRate, biomassMass])
    row1 += 1
  else:
    sheet2.write(row2, 0, tarYield['yield'])
    sheet2.write(row2, 1, tungstenHeatingRate)
    sheet2.write(row2, 2, biomassMass)

    datasetB.append([tarYield['yield'], tungstenHeatingRate, biomassMass])
    row2 += 1

  alternatingIndex += 1
  sheet3.write(row3, 0, tarYield['yield'])
  sheet3.write(row3, 1, tungstenHeatingRate)
  sheet3.write(row3, 2, biomassMass)
  row3 += 1

  print('rows processed: ' + str(row3) + ', tungstenHeatingRate=' + str(tungstenHeatingRate))
